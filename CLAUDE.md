...
- **frontend/Web は「ドキュメント < 実挙動」**（技術の進歩が速い前提で扱う）:
  - **ドキュメントを鵜呑みにしない**。仕様・API の挙動が要点になる時は、この
    repo 上で**実際に動かして**確かめる（最小再現 / test / 実行ログのいずれか）。
  - **「前に動いた・昔こう書いた」を根拠に過信しない**。過去の正解は現在の正解を
    保証しない（バージョン差・破壊的変更をまず疑う）。
  - **検証が要るものは、この repo 上で報告してから確定する**（推測で先に進めない）。
  - **本番に作り込む前に POC で先に確かめてよい**（むしろ推奨）。不確実な技術は
    小さく試して当たりを付けてから本実装へ——**手戻りの方が高くつく**。
