...
const task = (id: string, status: string, priority: number, extra: Partial<Task> = {}): Task => ({
  id,
  title: `title of ${id}`,
  status,
  priority,
...
const cardsIn = (lane: string) =>
  within(screen.getByRole("region", { name: lane })).queryAllByTestId("task-card");
...
describe("BoardView", () => {
  it("renders one column per board lane, in board order, empty lanes included", async () => {
    ...
    expect(columns.map((c) => c.getAttribute("aria-label"))).toEqual(["backlog", "ready", "done"]);
  ...
  it("orders cards by sparse priority within a column (lower = higher up)", async () => {
    ...
    expect(cardsIn("ready").map((c) => c.dataset.taskId)).toEqual(["t-2", "t-1"]);
  });
  ...
  it("flags and dims a blocked card, naming its blockers", async () => {
    ...
    expect(within(blocked!).getByLabelText("blocked by t-9")).toBeInTheDocument();
